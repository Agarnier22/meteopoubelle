...
arrondissement = [
    "13001", "13002", "13003", "13004", "13005", "13005", "13006", "13007", "13008", "13009",
...
if wind_type_choice == "Mistral":
    df = df[df['direction'].isin(['NNO', 'O', 'NO', 'ONO', 'N'])]
elif wind_type_choice == "Sirocco":
    df = df[df['direction'].isin(['SSE', 'SE', 'E', 'S', 'ESE'])]
...
if data_count < (30 * city_parts_number):
    color = "red"
...
df_grouped = df[[
    ...
    "NB_DECHET_SECTEUR_ALIMENTATION": "Alimentaire"})
...
df_grouped.plot(kind='barh', stacked=True, x="TYPE_LIEU_V2_1", y=[
    "Tabac", "Pharmaceutique", "Alimentaire"], ax=ax, cmap="Set3", ylabel="")
